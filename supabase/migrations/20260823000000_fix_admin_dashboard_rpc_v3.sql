-- Migration: consolida correções pendentes da RPC do Intelligence Center
-- (auditoria de 23/08/2026)
--
-- Esta migration formaliza uma correção que havia ficado perdida:
-- o arquivo 20260819212032_fix_admin_dashboard_identity_rpc.sql foi
-- commitado vazio, e a correção real (troca de TEMP TABLE por TEMP VIEW,
-- para evitar bug de cache de plano de execução do Postgres em funções
-- SECURITY DEFINER chamadas repetidamente) ficou solta, não commitada,
-- em rewrite_rpc.sql na raiz do projeto.
--
-- Além de formalizar essa correção, esta migration também resolve:
--   1. Resolução de identidade inconsistente: testimonials/testified/responded
--      não passavam pela resolução hist_canonical_id/period_canonical_id
--      que já era aplicada em active_users/reads/shares/funil inicial.
--   2. "Top Conteúdos" retornando apenas UUID: agora inclui o título do
--      devocional (idioma fonte) via JOIN com a tabela devotionals.
...
BEGIN
    ...
    -- =========================================================
    -- TEMPORARY VIEW FOR RESOLVED IDENTITIES
    -- =========================================================
    -- Usamos VIEW (não TABLE) para evitar o bug de cache de plano de
    -- execução do Postgres em TEMP TABLEs dentro de funções SECURITY
    -- DEFINER chamadas repetidamente na mesma sessão.
    DROP VIEW IF EXISTS tmp_resolved_events;
    CREATE TEMP VIEW tmp_resolved_events AS
    ...
    -- BUGFIX: usa hist_canonical_id (era user_id bruto), consistente com as demais métricas.
    SELECT count(DISTINCT hist_canonical_id) INTO v_curr_testimonials
    ...
    SELECT count(DISTINCT hist_canonical_id) INTO v_prev_testimonials
    ...
    -- BUGFIX: usa period_canonical_id (era user_id bruto), consistente com as etapas anteriores do funil.
    SELECT count(DISTINCT period_canonical_id) INTO v_funnel_testified
    FROM tmp_resolved_events WHERE event_type = 'testimonial_submitted' AND occurred_at::date BETWEEN p_start_date AND p_end_date;

    SELECT count(DISTINCT period_canonical_id) INTO v_funnel_responded
    FROM tmp_resolved_events WHERE event_type = 'testimonial_responded' AND occurred_at::date BETWEEN p_start_date AND p_end_date;

    SELECT count(DISTINCT r.period_canonical_id) INTO v_funnel_returned
    FROM tmp_resolved_events r
    JOIN tmp_resolved_events resp ON resp.period_canonical_id = r.period_canonical_id
    ...
    v_result := jsonb_build_object(
        ...
        -- BUGFIX: agora inclui devotional_title via JOIN, em vez de só o UUID cru.
        -- content_id continua NULL para eventos antigos (gravados antes da
        -- correção do AnalyticsService.trackEvent) — isso é esperado e não
        -- afeta eventos novos.
        'top_content', (
            ...
            FROM (
                SELECT
                    ...
                    count(*) as opens
                FROM tmp_resolved_events e
                LEFT JOIN public.devotionals d ON d.id = e.content_id
                WHERE e.event_type = 'devotional_opened'
                  AND e.occurred_at::date BETWEEN p_start_date AND p_end_date
                  AND e.content_id IS NOT NULL
                GROUP BY e.content_id, d.title
                ORDER BY opens DESC
                LIMIT 5
            ) t
        ),
