...
-- Edge function uses service role key and bypasses RLS; admin policies added for future panel.

-- 1. api_applications — one record per partner organization
...
-- 2. api_keys — each application can have multiple keys (rotation, env separation)
CREATE TABLE public.api_keys (
  id                  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  application_id      UUID        NOT NULL REFERENCES public.api_applications(id) ON DELETE CASCADE,
  key_hash            TEXT        NOT NULL UNIQUE, -- SHA-256 of the full key, never stored in plain text
  key_prefix          TEXT        NOT NULL,        -- first 12 chars of the key (e.g. "3mfl_abc123x"), for display only
  name                TEXT,                        -- label: "Production", "Staging"
  is_active           BOOLEAN     NOT NULL DEFAULT true,
...
-- 3. api_request_logs — one row per API request, used for rate limiting and auditing
...
-- Index for rate limit queries (count recent requests per key)
CREATE INDEX idx_api_request_logs_key_time
  ON public.api_request_logs (api_key_id, created_at DESC);

-- Index for audit queries per application
