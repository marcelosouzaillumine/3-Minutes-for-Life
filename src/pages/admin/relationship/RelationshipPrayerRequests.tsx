...
import type { AdminPrayerRequestItem, PaginatedResult } from '../../../types/Relationship';

export function RelationshipPrayerRequests() {
  const { t } = useTranslation(['common']);
  const [result, setResult] = useState<PaginatedResult<AdminPrayerRequestItem> | null>(null);
  ...
  const fetchPrayerRequests = async () => {
    ...
    setHasError(false);
    try {
      const data = await AdminRelationshipService.getPrayerRequests({
        status,
        language,
        period,
        search: searchTerm,
  ...
  const getStatusBadge = (s: string) => {
    switch (s) {
      ...
      case 'prayed':
        return <span style={{ padding: '2px 8px', borderRadius: '12px', background: '#dbeafe', color: '#1e40af', fontSize: '0.75rem', fontWeight: 600 }}>{t('admin.relationship.status.prayed', 'Em oração')}</span>;
      case 'archived':
  ...
  return (
    <div>
      <div className="admin-header">
        <div>
          <h1>{t('admin.relationship.prayerRequestsTitle', 'Gestão de Pedidos de Oração')}</h1>
          <p>{t('admin.relationship.prayerRequestsSubtitle', 'Receba e acompanhe os pedidos de oração enviados com confiança pela comunidade.')}</p>
        </div>
      </div>

      <RelationshipNav activeTab="prayer_requests" />
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            >
              ...
              <option value="prayed">{t('admin.relationship.status.prayed', 'Em oração')}</option>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-light)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              {t('admin.relationship.columns.language', 'Idioma')}
            </label>
            <select
              value={language}
            ...
            >
              <option value="all">{t('admin.relationship.filters.allLanguages', 'Todos os Idiomas')}</option>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <form onSubmit={handleSearchSubmit} style={{ display: 'flex', gap: '8px' }}>
              <div style={{ position: 'relative', flex: 1 }}>
                <input
                  type="text"
                  placeholder={t('admin.relationship.searchPrayerPlaceholder', 'Buscar no pedido...')}
                  value={searchTerm}
        ...
        {(status !== 'all' || language !== 'all' || period !== 'all' || searchTerm.trim()) && (
      ...
      {isLoading ? (
        <div className="admin-loading-state" style={{ minHeight: '200px' }}>
          ...
          <p>{t('admin.relationship.loadingPrayers', 'Carregando pedidos de oração...')}</p>
        </div>
      ) : hasError ? (
        <div className="admin-error-state">
          <p>{t('admin.relationship.errorLoading', 'Erro ao carregar pedidos de oração.')}</p>
          <button onClick={fetchPrayerRequests} style={{ marginTop: '8px', padding: '6px 12px', background: 'var(--color-accent)', border: 'none', color: '#fff', borderRadius: '4px', cursor: 'pointer' }}>
            {t('admin.relationship.tryAgain', 'Tentar Novamente')}
          </button>
        </div>
      ) : !result || result.data.length === 0 ? (
        <div className="admin-empty-state" style={{ minHeight: '200px' }}>
          {status !== 'all' || language !== 'all' || searchTerm ? t('admin.relationship.noResultsFiltered', 'Nenhum registro corresponde aos filtros selecionados.') : t('admin.relationship.noPrayerRequests', 'Ainda não recebemos pedidos de oração.')}
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)', backgroundColor: 'var(--color-bg-secondary, rgba(0,0,0,0.02))' }}>
                ...
                <th style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-light)' }}>{t('admin.relationship.columns.language', 'Idioma')}</th>
                <th style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--color-text-light)' }}>{t('admin.relationship.columns.preview', 'Pedido')}</th>
            ...
            <tbody>
              {result.data.map((item) => (
                ...
                >
                  ...
                  <td data-label={t('admin.relationship.columns.language', 'Idioma')} style={{ padding: '12px 16px', color: 'var(--color-text-light)', textTransform: 'uppercase', fontSize: '0.8rem', fontWeight: 500, whiteSpace: 'nowrap' }}>
                    {item.language}
                  </td>
                  <td data-label={t('admin.relationship.columns.preview', 'Pedido')} style={{ padding: '12px 16px', color: 'var(--color-text)', maxWidth: '280px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.request}
                  </td>
                  ...
                  <td data-label={t('admin.relationship.columns.action', 'Ação')} style={{ padding: '12px 16px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <button
                      onClick={() => setSelectedPrayerRequest(item)}
                      style={{
                        padding: '6px 12px',
                        borderRadius: '6px',
                        border: '1px solid var(--color-border)',
                    ...
                    >
                      {t('admin.relationship.actions.viewPrayer', 'Ver pedido')}
                    </button>
                  </td>
                </tr>
              ))}
      ...
      <PrayerRequestDetailModal
        ...
        onStatusUpdated={fetchPrayerRequests}
      />
    </div>
  );
}
