...
function runSql(sql: string) {
  try {
    ...
    return { error: null, output: out.toString().trim() };
  } catch (err: any) {
    return { error: err.stdout ? err.stdout.toString().trim() : err.message, output: null };
  }
}
...
async function runTests() {
  ...
  // Create users via SQL with a known password hash using pgcrypto
  // This bypasses auth rate limits while allowing normal sign-in
  console.log("Seeding test users via raw SQL...");
  runSql(`
    INSERT INTO auth.users (instance_id, id, aud, role, email, encrypted_password, email_confirmed_at, raw_app_meta_data, raw_user_meta_data, is_super_admin, created_at, updated_at, confirmation_token, email_change, email_change_token_new, recovery_token) 
    VALUES 
    ('00000000-0000-0000-0000-000000000000', '${uidA}', 'authenticated', 'authenticated', 'u1_${uidA}@test.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', false, now(), now(), '', '', '', ''),
    ('00000000-0000-0000-0000-000000000000', '${uidB}', 'authenticated', 'authenticated', 'u2_${uidB}@test.com', crypt('password123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', false, now(), now(), '', '', '', '');
    
    INSERT INTO public.profiles (id, full_name) VALUES ('${uidA}', 'User A'), ('${uidB}', 'User B');
    
    INSERT INTO public.supporters (user_id, status) VALUES ('${uidA}', 'active'), ('${uidB}', 'active');
  ...
  console.log("Seeded successfully. Running JS client tests...");
  ...
  // Login User A
  const { error: signInErr } = await supabase.auth.signInWithPassword({ email: `u1_${uidA}@test.com`, password: 'password123' });
  if (signInErr) {
  ...
  if (ownContribId) {
    ...
    record('G3 - CONTRIBUTION TAMPERING (amount)', 'User A', 'UPDATE', 'contributions', 'Error or 0 rows updated', res.error ? res.error.message : 'Silently ignored', res.error || (res.data?.length===0) || res.status===204 ? 'PASS' : 'FAIL');
    
    res = await supabase.from('contributions').delete().eq('id', ownContribId);
    record('G3 - CONTRIBUTION TAMPERING (delete)', 'User A', 'DELETE', 'contributions', 'Error or 0 rows deleted', res.error ? res.error.message : 'Silently ignored', res.error || res.status===204 ? 'PASS' : 'FAIL');
  } else {
  ...
  record('Explicit 3', 'User A', 'UPDATE', 'supporters.status', 'Error/0 rows', res.error ? res.error.message : 'Silently ignored', res.error || res.status===204 ? 'PASS' : 'FAIL');
  ...
  record('Explicit 4', 'User A', 'UPDATE', 'supporters.user_id', 'Error/0 rows', res.error ? res.error.message : 'Silently ignored', res.error || res.status===204 ? 'PASS' : 'FAIL');

  // Audit Grants & Policies
  console.log("Auditing grants and policies...");
  const grantsSql = `
    SELECT table_name, grantee, privilege_type 
    FROM information_schema.role_table_grants 
    WHERE table_name IN ('supporters', 'contributions', 'payment_events', 'campaigns') AND grantee IN ('anon', 'authenticated');
  `;
  const grantsOut = runSql(grantsSql).output;
  console.log("GRANTS:\n" + grantsOut);

  const policiesSql = `
    SELECT tablename, policyname, roles, cmd 
    FROM pg_policies 
    WHERE tablename IN ('supporters', 'contributions', 'payment_events', 'campaigns');
  `;
  const policiesOut = runSql(policiesSql).output;
  ...
  for (const m of matrix) {
    ...
    md += `| ${m.test} | ${m.actor} | ${m.operation} | ${m.target} | ${m.expected} | ${cleanActual} | **${m.result}** |\n`;
  }
  ...
  // Cleanup auth users manually
  runSql(`DELETE FROM auth.users WHERE id IN ('${uidA}', '${uidB}')`);
}
